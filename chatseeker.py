...
def _classify_ytdlp_error(combined: str) -> str:
    ...
    Urutan pengecekan SENGAJA dari paling spesifik → paling umum, supaya
    pesan yang kebetulan mengandung beberapa keyword sekaligus tetap
    kena kategori yang paling akurat (bukan ke-overwrite kategori umum).
    """
    c = combined.lower()

    # 1. Video benar-benar hilang/dihapus/private — gak ada hubungannya dgn auth
    if "video unavailable" in c or "private video" in c or "this video is not available" in c:
        return "unavailable"

    # 2. Age-restricted — butuh cookies akun 18+ yang valid, beda solusi dari #1
    if "confirm your age" in c or "age-restricted" in c or "age restricted" in c:
        return "age_restricted"

    # 3. Bot-check YouTube — butuh cookies login valid (baru, sejak pertengahan 2025)
    ...
    # 6. Baru di sini "no live chat" — paling umum, dicek PALING TERAKHIR
    #    supaya gak jadi catch-all yang salah nangkep error lain.
    if "no subtitles" in c or "there are no" in c or "subtitles not available" in c \
            or "no live_chat" in c:
        return "no_chat"
            ...
            for future in as_completed(futures):
                ...
                with _stats_lock:
                    ...
                    if status == "ok":
                        ...
                        _stats["age_restricted"] += 1
                        consecutive_rl = 0
                    elif status == "auth_required":
                        _stats["auth_required"] += 1
                        consecutive_rl = 0
                ...
                icon_map = {
                    ...
                    "age_restricted": "[orange3]🔞[/orange3]",
                    "auth_required":  "[magenta]🔑[/magenta]",
